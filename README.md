## Definitely not a blog

_Please enjoy responsibly._

## 💻 Dev
- [**DIY Autoscaling on Railway with ~150 Lines of Python**](articles/dev/railway-autoscaling.md#diy-autoscaling-on-railway-with-150-lines-of-python)<br>
_Building a lightweight autoscaler that scales worker replicas based on job backlog using Railway's GraphQL API_  
  <sub>Last updated: 2026-03-28</sub>
- [**Tart Cheatsheet**](articles/dev/tart.md#tart-cheatsheet)<br>
_Easy macOS VMs_  
  <sub>Last updated: 2026-08-15</sub>

---


&nbsp;
## 🎸 Music
- [**Recording Mac audio out**](articles/music/recording_mac_audio_output.md#scenario)<br>
_How to record both your instrument and computer audio simultaneously_  
  <sub>Last updated: 2025-07-13</sub>

---


&nbsp;